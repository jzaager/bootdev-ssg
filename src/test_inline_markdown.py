...
class TestTextToNodes(unittest.TestCase):
    ...
    def test_only_bold(self):
        input_text = "This is test input text with only **bolded** text and no other formats."
        self.assertEqual(
            text_to_text_nodes(input_text),
            [
                TextNode("This is test input text with only ", TextType.TEXT),
                TextNode("bolded", TextType.BOLD),
                TextNode(" text and no other formats.", TextType.TEXT)
            ]
        )

    def test_italic_b4_bold(self):
        input_text = "This is test input text with *only* **bolded** text and no other formats."
        self.assertEqual(
            text_to_text_nodes(input_text),
            [
                TextNode("This is test input text with ", TextType.TEXT),
                TextNode("only", TextType.ITALIC),
